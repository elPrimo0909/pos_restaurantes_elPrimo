...
$impuestotem2= strlen($impuestos);

if($impuestotem2<2){

	$impuestos100="0"."$impuestos";
}else{

	$impuestos100="$impuestos";
}
...
$mesa2=0;
$sql_mesa = mysqli_query($con, "select * from ostemporal_mesas2 order by id DESC limit 1");
$rw_promesas  = mysqli_fetch_array($sql_mesa);
  $mesa2=$rw_promesas['titulo'];

 $sql_eeee = mysqli_query($con, "select * from ostemporal_cotizaciones where titulo ='$articulo' AND mesa='$mesa2' "  );
 $rw_proeeees  = mysqli_fetch_array($sql_eeee);

  $mesa5=$rw_proeeees['mesa'];


$sql_oste = mysqli_query($con, "select * from ostemporal_cotizaciones where titulo ='$articulo' AND mesa='$mesa2'");
...
if($mesa2===""){
echo "<br>";
	echo "<h2>Selecciona una Mesa</h2>";
...
switch (true) {
    case $mesa2===$mesa5:
		$sql5        = "UPDATE ostemporal_cotizaciones SET come='".$cantidad2."' WHERE titulo='".$articulo."' AND mesa='$mesa2'";
		$query_update = mysqli_query($con, $sql5);
		
        break;
    default:
		$insert_tmp = mysqli_query($con, "INSERT INTO ostemporal_cotizaciones (id,titulo,come,celu,num,estado,impuesto,descuento,mesa) VALUES (null,'$articulo','$cantidad','$session_id','$prec44','$codigo','$impuestos100','$descuentoproducto','$mesa2')");
...
$sql_mesa = mysqli_query($con, "select * from ostemporal_mesas2 order by id DESC limit 1");
$rw_promesas  = mysqli_fetch_array($sql_mesa);
 $mesa=$rw_promesas['titulo'];

$sql = mysqli_query($con, "select * from ostemporal_cotizaciones  where mesa='".$mesa."'  ORDER by id DESC ");
...
$porc=0;
	if($impuesto1==$hola) {
		$porc=0;
			...
			$impuestotem21= strlen($impuesto1);

			if($impuestotem21<2){
			
				$impuestos1001="0"."$impuesto1";
			}else{
			
				$impuestos1001="$impuesto1";
			}
...
$impuesto4 = "$raiz_im" . "$impuestos1001"; 
